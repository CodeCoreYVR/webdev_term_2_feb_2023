// How to use promises before creating our ownx
const fs = require('fs').promises // all the same functions available, but just as promises

let p = fs.readFile('./file-to-read/file1.txt', 'utf-8') // return a promise
...
p.then((data) => {
    console.log(data)
    return fs.readFile('./file-to-read/file2.txt', 'utf-8')
}).then((data) => {
    console.log(data)
    return fs.readFile('./file-to-read/file3.txt', 'utf-8')
}).then((data) => {
    console.log(data)
})
