...
const server = http.createServer((req, res) => {
  ...
  if (url === '/calculate') {
    ...
    // parse the query parameters and extract num1, operator, and num2
    ...
    switch (operator) {
      ...
      case '/':
        result = num1 / num2;
        break;
      // if an invalid operator is specified, write an error message and end the response
      default:
        res.write(`<h1>Error: Invalid operator ${operator}.</h1>`);
        return res.end();
    }
  
    // write the calculation result to the response
    res.write(`
      <head>
        <link rel="stylesheet" type="text/css" href="stylesheet/style.css">
      </head> 
